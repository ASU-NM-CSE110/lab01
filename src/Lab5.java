...
//FILENAME: Lab5.java
//SPECIFICATION: This lab will ask for user input and and depending on the input show either a row, pyramid, or a diamond
//FOR: CSE 110 
...
public class Lab5 {
 ...
 private static String buildDiamond(
     ...
     String result = "";
     if(numSymbols % 2 == 0) {
    	 System.out.println("The input is not an odd number. Use the closest odd number below " + numSymbols); // will make any even number into odd number
    	 numSymbols--;
     }
     
     int totalSymbOneRow = numSymbols;

     for (int numStars = 1; numStars <= numSymbols; numStars += 2) { // building top half of the diamond 
         int numDashed = (totalSymbOneRow - numStars) / 2;
         String row = buildRow(sideSymb, numDashed, midSymb, numStars);
         result += row + "\n";
     }
     for (int numStars = totalSymbOneRow - 2; numStars >= 0; numStars -= 2) { //modified version of the top half but instead the numStars are subtracting and it keeps repeating until 0
         int numDashed = (totalSymbOneRow - numStars) / 2;
         String row = buildRow(sideSymb, numDashed, midSymb, numStars);
         result += row + "\n"; // return row
     }
...
/* Output: Please choose one pattern from the list:
r) Row
p) Pyramid
d) Shallow diamond
q) Quit
